...
```{r}
df1 %>% filter(dispub92 %in% c("01","02","03","04","05","09","20","50","51","62","65","70")) %>% ggplot(aes(x = dispub92, color = sex1))+
  geom_bar()+
  labs(title = 'Bar graph of frequency of discharges stratified by sex')
```

```{r}
library(ggplot2)
df1 %>% ggplot() + geom_point(mapping=aes(x=age, y=los)) + facet_wrap(~sex1) +
  labs(x='Age', 
       y = 'Length of Stay', 
...
```{r}
df1 %>% ggplot() + geom_point(mapping=aes(x=age, y=tot)) + facet_wrap(~sex1) +
  labs(x='Age', 
       y = 'total expenses', 
       title = 'Length of Stay by Age and total expenses', 
       caption = '')
```

```{r}
library(ggplot2)
df1 %>% ggplot() + geom_point(mapping=aes(x=los, y=tot)) + facet_wrap(~provider1) +
...
```{r}
#teenagers
df1 %>% filter(age <= 20) %>% ggplot(aes(y = provider1,
           x = tot, color=sex1))+
  geom_col()+
  labs(title = 'provider by expense by gender')
```

```{r}
df1 %>% ggplot(aes(x = moa,y= tot,
            color=sex1))+
  geom_col()+
  labs(title = 'moa vs total room and board')
```
...
df1 %>% ggplot(aes(x = campus,
           y = tot, color=sex1))+
  geom_col()+
  labs(title = 'total expenses by admin type')
```

```{r}

df1 %>% ggplot() + geom_point(mapping=aes(x=los, y=tot, color = sex1)) + facet_wrap(~provider1) +
  labs(x='Length of Stay', 
       y = 'Total Charge', 
       title = 'Total Charge by Length of Stay and Provider', 
       caption = '')
...
```{r}
df1 %>% filter(pt_state %in% c("MA", "RI", "CT")) %>% 
ggplot(aes(x = pt_state,y = tot
           ))+
  geom_col()+
  labs(title = 'pysical therapy in MA, CT, RI')
```

```{r}
df1 %>% ggplot() + geom_col(mapping=aes(y=provider1,x = tot, fill=season)) +
...
library(ggplot2)
library(tidyverse)
df1 %>% ggplot() + geom_bar(mapping = aes(x = sex, fill = trandb)) + 
