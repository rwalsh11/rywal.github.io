...
```{r}
df1 %>% group_by(campus, sex1) %>% summarise(mean = mean(tot)) %>% ggplot(aes(x = campus,
           y = mean, color=sex1))+
  geom_col()+
...
graph %>% ggplot(aes(x = pt_state, y = mean
           ))+
  geom_col()+
  labs(title = 'pysical therapy in MA, CT, RI', x = "Physical therapy location", y = " average total expenses", caption = "average expenses in Massachusetts appear higher than onneticut C Rhode Island")
```
